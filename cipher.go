...
func NewCipher(key []byte, hashf HashFunction) *Cipher {
	...
	dkey = prng.hash(dkey)
	dhash := prng.hash(dkey)
	dprng := NewPRNG(dhash[:], hashf)
	
	sboxes_e := [][]byte{
		[]byte{0x0,0xE,0xB,0x1,0x4,0x2,0xD,0x3,0xF,0x7,0x5,0xC,0x6,0x8,0xA,0x9},
		[]byte{0xF,0xD,0xB,0x7,0xC,0xE,0xA,0x3,0x9,0x8,0x6,0x2,0x4,0x1,0x5,0x0},
		[]byte{0x1,0x0,0x3,0x2,0x5,0x4,0x7,0x6,0x9,0x8,0xB,0xA,0xD,0xC,0xF,0xE},
		[]byte{0x2,0x5,0x0,0x1,0x8,0x3,0x4,0xA,0xB,0x7,0x9,0xE,0xF,0xC,0xD,0x6},
		[]byte{0xE,0x6,0x0,0x2,0x3,0x1,0x4,0x5,0x7,0x8,0xF,0xA,0xB,0x9,0xC,0xD},
		[]byte{0x7,0xB,0x1,0x3,0x4,0x5,0x2,0xA,0x0,0x8,0xC,0x9,0xD,0x6,0xF,0xE},
		[]byte{0xC,0xF,0xB,0x9,0x5,0x3,0x0,0x2,0x4,0x6,0x7,0x8,0xA,0xD,0xE,0x1},
		[]byte{0x7,0x9,0x6,0xE,0x2,0xC,0x4,0x1,0x0,0x3,0x5,0xF,0x8,0xA,0xD,0xB},
	}
	
	// Compute the inverses
	sboxes_d := make([][]byte, len(sboxes_e))
	
	for i := 0; i < len(sboxes_e); i++ {
		sbox_d := make([]byte, len(sboxes_e[i]))
		
		for j := 0; j < len(sbox_d); j++ {
			sbox_d[sboxes_e[i][j]] = byte(j)
		}
